...
// 显示恢复播放提示
function showResumePlayPrompt(videoTitle) {
  const prompt = document.createElement('div');
  prompt.className = 'resume-play-prompt';
  prompt.innerHTML = `
        <div class="prompt-content">
            <span>继续观看: ${videoTitle}</span>
            <button id="resume-play-btn">继续</button>
            <button id="dismiss-resume-btn">关闭</button>
        </div>
    `;
  document.body.appendChild(prompt);

  // 继续播放按钮事件
  document.getElementById('resume-play-btn').addEventListener('click', () => {
  ...
  if (hasAdultSelected) {
    ...
    if (filterDescription) {
      filterDescription.innerHTML =
        '<strong class="text-pink-300">选中test时无法启用此过滤</strong>';
    }
