...
async function handleApiRequest(url) {
    ...
    try {
        if (url.pathname === '/api/search') {
            ...
            // 提取重复的 HTML 请求和处理逻辑
async function fetchAndProcessHtml(detailUrl) {
    ...
    try {
        ...
        return { matches, titleText, descText };
    } catch (error) {
        clearTimeout(timeoutId);
        throw error;
    }
...
async function handleCustomApiSpecialDetail(id, customApi) {
    try {
        ...
        const { matches, titleText, descText } = await fetchAndProcessHtml(detailUrl);
...
async function handleSpecialSourceDetail(id, sourceCode) {
    try {
        ...
        if (sourceCode === 'ffzy') {
            ...
            const ffzyMatches = detailHtml.match(ffzyPattern) || [];
            if (ffzyMatches.length > 0) {
                matches = ffzyMatches.map(link => {
                    link = link.substring(1, link.length);
                    const parenIndex = link.indexOf('(');
                    return parenIndex > 0 ? link.substring(0, parenIndex) : link;
                });
