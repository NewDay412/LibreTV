...
function saveSearchHistory(query) {
  ...
  history = history.filter(
    (item) => typeof item === 'object' && item.timestamp && now - item.timestamp < 5184000000
  );
...
async function playFromHistory(url, title, episodeIndex, playbackPosition = 0) {
  ...
  try {
      ...
      try {
        ...
        if (videoDetails && videoDetails.episodes && videoDetails.episodes.length > 0) {
          ...
          if (newEpisodeCount > oldEpisodeCount) {
            showToast(
              `已同步最新剧集列表 (${newEpisodeCount}集，新增${newEpisodeCount - oldEpisodeCount}集)`,
              'success'
            );
    ...
    const sourceCodeForUrl = historyItem
      ...
      : new URLSearchParams(new URL(url, window.location.origin).search).get('source_code');
